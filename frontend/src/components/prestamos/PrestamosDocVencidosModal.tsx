...
import { GET_ALL_PRESTAMOS_DOC_VENCIDOS_PAGINATED } from '../../lib/queries';

interface PrestamoDocVencido {
  id: string; fechaPrest: string; fechaDevolucion: string;
  persona: { id: string; nombre: string; apellido: string; ci: string };
  items: { id: string; estado: string; documento: { id: string; codigoDoc: string; titulo: string } }[];
}

interface Props {
  onClose: () => void;
  onSelect: (p: PrestamoDocVencido) => void;
  formatDate: (d: string) => string;
}

const ITEMS_PER_PAGE = 10;

export default function PrestamosDocVencidosModal({ onClose, onSelect, formatDate }: Props) {
  const [page, setPage] = useState(0);
  const { data, loading } = useQuery(GET_ALL_PRESTAMOS_DOC_VENCIDOS_PAGINATED, {
    variables: { page: page + 1, pageSize: ITEMS_PER_PAGE },
    fetchPolicy: 'network-only',
  });

  const items = data?.allPrestamosDocVencidosPaginated?.items ?? [];
  const totalCount = data?.allPrestamosDocVencidosPaginated?.totalCount ?? 0;
      ...
      <div className="glass-panel rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-xl font-bold text-surface-800 dark:text-navy-200 flex items-center gap-2">
              ...
              Préstamos de Documentos Vencidos
            </h3>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {items.map((p: PrestamoDocVencido) => {
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-xs text-surface-600 dark:text-navy-500 mt-1 space-y-0.5">
                        ...
                        <p>CI: {p.persona.ci} · {p.items.length} documento(s)</p>
                      </div>
                    </div>
